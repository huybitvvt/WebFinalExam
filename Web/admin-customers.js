const mockCustomerSeedVersion = 'dngear-customer-seed-v4';

function buildMockCustomers() {
    return [
        { username: 'khach123', name: 'Khách VIP 1', email: 'khach123@gmail.com', password: '12', role: 'customer', spent: 0, status: 'active' },
        { username: 'hoangnam', name: 'Nguyễn Hoàng Nam', email: 'nam@tacticalmail.vn', password: '123', role: 'customer', spent: 0, status: 'active' },
        { username: 'minhkhoi', name: 'Trần Minh Khôi', email: 'khoi.blaster@gmail.com', password: '123', role: 'customer', spent: 0, status: 'active' },
        { username: 'quocbao', name: 'Lê Quốc Bảo', email: 'bao.qb@storemail.vn', password: '123', role: 'customer', spent: 0, status: 'active' },
        { username: 'thuha', name: 'Võ Thu Hà', email: 'ha.vo@buyer.vn', password: '123', role: 'customer', spent: 0, status: 'active' },
        { username: 'nhatlong', name: 'Phạm Nhật Long', email: 'long.modder@gmail.com', password: '123', role: 'customer', spent: 0, status: 'active' },
        { username: 'minhquan', name: 'Đỗ Minh Quân', email: 'quan.foam@gmail.com', password: '123', role: 'customer', spent: 0, status: 'active' },
        { username: 'gialinh', name: 'Nguyễn Gia Linh', email: 'linh.nguyen@order.vn', password: '123', role: 'customer', spent: 0, status: 'active' },
        { username: 'anhdung', name: 'Trần Anh Dũng', email: 'dung.field@buyer.vn', password: '123', role: 'customer', spent: 0, status: 'active' },
        { username: 'khanhvan', name: 'Phan Khánh Vân', email: 'vankhanh@shopmail.vn', password: '123', role: 'customer', spent: 0, status: 'active' },
        { username: 'thanhvy', name: 'Võ Thanh Vy', email: 'vy.vo@community.vn', password: '123', role: 'customer', spent: 0, status: 'active' }
    ];
}
...
function initMockCustomers() {
    ...
    // Chỉ bổ sung seed còn thiếu, không xóa các tài khoản khách đã tự đăng ký
    if (localStorage.getItem('customerSeedVersion') !== mockCustomerSeedVersion) {
        const legacySeedUsernames = new Set([
            'johnwick',
            'leon',
            'paul',
            'geralt',
        ...
        users = users.filter(user => !(user.role === 'customer' && legacySeedUsernames.has(String(user.username || '').trim().toLowerCase())));

        const seedCustomers = buildMockCustomers();
        seedCustomers.forEach(seedCustomer => {
            const existingIndex = users.findIndex(user =>
                user.email === seedCustomer.email
                || user.username === seedCustomer.username
            );
    ...
    if (typeof syncCustomerMetricsFromTransactions === 'function') {
        syncCustomerMetricsFromTransactions();
    }
}
initMockCustomers();
...
window.sendVoucher = function(email, name) {
    ...
    const activePromotions = typeof getPromotionData === 'function'
        ? getPromotionData().filter(promotion => promotion.status === 'Đang áp dụng')
        : [];
    ...
    vouchers.unshift({
        ...
        code: voucherCode,
        rankValue: Number(basePromotion.rankValue) || 10,
        rankUnit: basePromotion.rankUnit || '%',
        usageLimit: 1,
        uses: 0,
        status: 'Đang áp dụng',
