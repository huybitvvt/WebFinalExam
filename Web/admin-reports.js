...
const mockTransactionSeedVersion = 'dngear-transaction-seed-v3';

function createOrderItem(productId, quantity) {
    const product = typeof findProductById === 'function'
        ? findProductById(productId)
        : dbProducts.find(item => item.id === productId);
...
function buildCustomerUsernameSeed(name = '', email = '') {
    const baseValue = email ? String(email).split('@')[0] : name;
    const normalizedBase = String(baseValue || 'khach')
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '')
    ...
    return normalizedBase || 'khach';
}

function buildUniqueCustomerUsername(name, email, users) {
    const existingUsernames = new Set((Array.isArray(users) ? users : []).map(user => String(user.username || '').trim().toLowerCase()));
    const base = buildCustomerUsernameSeed(name, email);
    ...
    return [
        {
            ...
            note: 'Đơn carbine đầu mùa, đã giao cùng combo ống ngắm và đạn gel.',
            items: [
                { productId: 'RFL02', quantity: 1 },
                { productId: 'ACC01', quantity: 1 },
                { productId: 'AMO01', quantity: 4 }
            ]
        },
        {
            id: 'HD250603',
            ...
            status: 'Hoàn tất',
            paymentStatus: 'Đã thanh toán',
            note: 'Khách mua shotgun để chơi nhóm, chốt thêm vỏ đạn dự phòng.',
            items: [
                { productId: 'SHT01', quantity: 1 },
                { productId: 'AMO01', quantity: 2 },
                { productId: 'DEF01', quantity: 1 }
            ]
        },
        {
            id: 'HD250720',
            ...
            status: 'Hoàn tất',
            paymentStatus: 'Đã thanh toán',
            note: 'Khách đặt online sau khi xem buổi phát trực tiếp giới thiệu dòng SMG.',
            items: [
                { productId: 'SMG01', quantity: 1 },
                { productId: 'ACC01', quantity: 1 },
                { productId: 'AMO01', quantity: 6 }
            ]
        },
        {
            id: 'HD250902',
            ...
            status: 'Hoàn tất',
            paymentStatus: 'Đã thanh toán',
            note: 'Đơn súng ngắn kèm ống ngắm để chụp ảnh và trưng bày sản phẩm.',
            items: [
                { productId: 'PST02', quantity: 1 },
                { productId: 'ACC01', quantity: 1 }
            ]
        },
        {
            ...
            status: 'Hoàn tất',
            paymentStatus: 'Đã thanh toán',
            note: 'Combo AK chơi gel, kèm ống ngắm và đạn cho nhóm đi sân.',
            items: [
                { productId: 'RFL02', quantity: 1 },
                { productId: 'ACC01', quantity: 1 },
                { productId: 'AMO01', quantity: 5 }
            ]
        },
        {
            id: 'HD251219',
            ...
            status: 'Hoàn tất',
            paymentStatus: 'Đã thanh toán',
            note: 'Khách chốt AWM và lấy thêm đạn cùng ống ngắm cho hoạt động đội nhóm.',
            items: [
                { productId: 'SNP01', quantity: 1 },
                { productId: 'AMO01', quantity: 6 },
                { productId: 'ACC01', quantity: 1 }
            ]
        },
        {
            ...
            status: 'Hoàn tất',
            paymentStatus: 'Đã thanh toán',
            note: 'Đã xác nhận giao SMG và đồ phòng vệ theo số lượng câu lạc bộ.',
            items: [
                { productId: 'SMG02', quantity: 1 },
                { productId: 'DEF01', quantity: 2 }
            ]
        },
        {
            id: 'HD260214',
            ...
            status: 'Hoàn tất',
            paymentStatus: 'Đã thanh toán',
            note: 'Khách cũ mua thêm shotgun đạn xốp, ống ngắm và đạn để nâng trải nghiệm.',
            items: [
                { productId: 'SHT01', quantity: 1 },
                { productId: 'AMO01', quantity: 3 },
                { productId: 'ACC01', quantity: 1 }
            ]
        },
        {
            ...
            customerEmail: 'khach123@gmail.com',
            createdAt: '2026-03-05T19:30:00',
            status: 'Chờ duyệt',
            paymentStatus: 'Chờ xác nhận',
            note: 'Khách cần admin gọi lại để xác thực ưu đãi súng ngắn và số lượng đạn đi kèm.',
            items: [
                { productId: 'PST01', quantity: 1 },
                { productId: 'AMO01', quantity: 4 }
            ]
        },
        {
...
function normalizeStoredOrderItem(item) {
    ...
    const product = (typeof findProductById === 'function' ? findProductById(item.productId) : null)
        || dbProducts.find(candidate => candidate.name === item.name);
    const rawPrice = Number(item.price);
    const normalizedPrice = Number.isFinite(rawPrice) ? Math.max(0, Math.round(rawPrice)) : null;

    if (!product) {
        return {
            productId: item.productId || `ARCHIVED-${Date.now()}`,
            name: item.name || 'Sản phẩm đã gỡ khỏi catalog',
    ...
    return {
        ...
        price: normalizedPrice !== null ? normalizedPrice : Number(product.price || 0),
        quantity
    };
}
...
function normalizeStoredOrder(order) {
    ...
    const subtotal = calculateOrderTotal(normalizedItems);
    const rawDiscount = Number(order.discount || 0);
    const discount = Number.isFinite(rawDiscount) ? Math.max(0, Math.min(subtotal, Math.round(rawDiscount))) : 0;
    const rawTotal = Number(order.total);
    const total = Number.isFinite(rawTotal) ? Math.max(0, Math.round(rawTotal)) : Math.max(0, subtotal - discount);
    ...
    return {
        ...
        subtotal,
        discount,
        discountCode: String(order.discountCode || '').trim().toUpperCase(),
        total
    };
}
...
function initMockReports() {
    ...
    if (!hasSeededCurrentVersion || transactions.length === 0) {
        const seedOrderIds = new Set(seedOrders.map(order => order.id));
        const customOrders = transactions.filter(order => !seedOrderIds.has(order.id));
        transactions = normalizeTransactionCollection([...customOrders, ...seedOrders]);
...
function saveTransactionData(orders) {
    ...
    syncCustomerMetricsFromTransactions(normalizedOrders);
...
function syncCustomerMetricsFromTransactions(orders = getTransactionData()) {
    const normalizedOrders = normalizeTransactionCollection(orders);
    let users = JSON.parse(localStorage.getItem('users')) || [];
    if (!Array.isArray(users)) users = [];

    const completedOrders = normalizedOrders.filter(order => order.status === 'Hoàn tất');
    const spendingByCustomer = new Map();
    const customerProfiles = new Map();

    completedOrders.forEach(order => {
        const key = buildCustomerLookupKey(order);
        if (!key) return;

        spendingByCustomer.set(key, (spendingByCustomer.get(key) || 0) + Number(order.total || 0));
    ...
    spendingByCustomer.forEach((spent, key) => {
        const existingIndex = nextUsers.findIndex(user => buildCustomerLookupKey(user) === key);
        const profile = customerProfiles.get(key) || {};

        if (existingIndex === -1) {
            nextUsers.push({
                username: buildUniqueCustomerUsername(profile.name, profile.email, nextUsers),
        ...
        nextUsers[existingIndex] = {
            ...nextUsers[existingIndex],
            role: 'customer',
            name: nextUsers[existingIndex].name || profile.name || 'Khách hàng',
            email: nextUsers[existingIndex].email || profile.email || '',
            spent: Math.round(spent)
        };
    });
    ...
    const updatedUsers = nextUsers.map(user => {
        ...
        const key = buildCustomerLookupKey(user);
        return {
            ...user,
            spent: Math.round(spendingByCustomer.get(key) || 0)
        };
    });
    ...
    if (currentUser && currentUser.role === 'customer') {
        const matchedUser = updatedUsers.find(user =>
            ...
            || buildCustomerLookupKey(user) === buildCustomerLookupKey(currentUser)
        );
    ...
    if (typeof renderAdminCustomers === 'function') {
        const searchInput = document.getElementById('adminCustomerSearchInput');
        renderAdminCustomers(searchInput ? searchInput.value : '');
    }
...
function reserveInventoryForOrder(order) {
    ...
    (order.items || []).forEach(item => {
        const product = typeof findProductById === 'function'
            ...
            : dbProducts.find(candidate => candidate.id === item.productId);
        if (!product) {
            missingItems.push(item.name || item.productId || 'Sản phẩm không rõ');
            return;
        }
    ...
    if (missingItems.length > 0) {
        return {
            ok: false,
            message: `Không đủ tồn kho để duyệt đơn: ${missingItems.join(', ')}`
        };
    }
...
function buildCategoryRevenue(orders) {
    ...
    orders.forEach(order => {
        const subtotal = Number(order.subtotal || calculateOrderTotal(order.items || []));
        const normalizedSubtotal = Number.isFinite(subtotal) ? Math.max(0, subtotal) : 0;
        const rawDiscount = Number(order.discount || 0);
        const discount = Number.isFinite(rawDiscount) ? Math.max(0, Math.min(normalizedSubtotal, rawDiscount)) : 0;
        const netRatio = normalizedSubtotal > 0 ? Math.max(0, (normalizedSubtotal - discount) / normalizedSubtotal) : 1;

        (order.items || []).forEach(item => {
            const label = productCategoryLabels[item.category] || 'Khác';
            const lineTotal = (item.price || 0) * (item.quantity || 0);
            totals[label] = (totals[label] || 0) + (lineTotal * netRatio);
        });
    });
...
window.recordCheckoutOrder = function(cartItems, voucher = null) {
    if (!Array.isArray(cartItems) || cartItems.length === 0) return null;

    const currentUser = JSON.parse(localStorage.getItem('currentUser')) || {};
    const orders = getTransactionData();
    const normalizedVoucher = typeof normalizeVoucherRecord === 'function' && voucher
        ? normalizeVoucherRecord(voucher, voucher.source || 'promotion')
        : voucher;
    ...
    const normalizedItems = cartItems.map(item => ({
        ...
        price: typeof getProductSellingPrice === 'function'
            ? getProductSellingPrice(item.product)
            : Number(item.product.price || 0),
        quantity: item.quantity
    }));
    const totals = typeof calculateCartTotals === 'function'
        ? calculateCartTotals(cartItems, normalizedVoucher)
        : {
            subtotal: normalizedItems.reduce((sum, item) => sum + (item.price * item.quantity), 0),
            discount: 0,
            finalTotal: normalizedItems.reduce((sum, item) => sum + (item.price * item.quantity), 0)
        };
    const order = {
        ...
        status: 'Chờ duyệt',
        paymentStatus: 'Chờ xác nhận',
        note: normalizedVoucher
            ? `Yêu cầu mới tạo từ giỏ hàng khách hàng. Đã áp dụng mã ${normalizedVoucher.code}.`
            : 'Yêu cầu mới tạo từ giỏ hàng khách hàng.',
        items: normalizedItems,
        subtotal: totals.subtotal,
        discount: totals.discount,
        discountCode: normalizedVoucher ? normalizedVoucher.code : '',
...
window.syncCustomerMetricsFromTransactions = syncCustomerMetricsFromTransactions;
