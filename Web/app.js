...
function renderSidebar() {
    ...
    if (nameDisplayEl) {
        nameDisplayEl.innerText = `Xin chào, ${currentUser.name || 'Người dùng'}`;
    }
    ...
    if (currentUser.role === 'admin') {
        if (roleLabel) roleLabel.innerText = 'Quản trị viên';
        ...
        if (roleLabel) roleLabel.innerText = 'Khách hàng';
...
function openPersonalInfo() {
    ...
    const targetItem = menuItems.find(item => item.textContent.includes('Thông tin cá nhân')) || null;
    switchMenu(targetItem, 'view-personal-info', 'renderPersonalInfo');
}
